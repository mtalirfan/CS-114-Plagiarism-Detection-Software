...
string similar_phrase_data[2][1000]; // DON'T CHANGE FIRST INDEX: index [0] for source file number, index [1] for phrase string, CAN CHANGE SECOND INDEX: this variable can store [1000] phrase strings and their file numbers
...
void word_check(ofstream& test_file) { // function of type void that does not return any value to main function, takes in an output stream parameter
    ...
    cout << endl << endl << setprecision(3) << "\t\tSimilarity Index = " << data_of_file.similarity_percentage[0] << "%" << endl;
    test_file << endl << endl << setprecision(3) << "\t\tSimilarity Index = " << data_of_file.similarity_percentage[0] << "%" << endl;
    for (i = 1; i <= 5; i ++) {
...
void phrase_check(ofstream& test_file) {
    ...
    string word, test_word, test_phrase, source_line, temp_phrase;
    ...
    for (i = 1; i <= 5; i++) { // for source files 1-5
        ...
        while ( file[0].good() ) {
            ...
            // cout << endl << test_words_done; // debugging
            ...
            while ( !file[i].eof() ) {
                ...
                if ( test_substring_position != string::npos ) {
                    test_words_done = test_words_done + min_phrase_len;
            ...
            if ( test_substring_position == string::npos ) {     
                test_words_done = test_words_done + 1;

                file[i].clear();
                file[i].seekg(0); // clear and seek to start for source file i      
                file[0].clear();
                file[0].seekg(0); // clear and seek to start for source file i
                continue;
            }
            ...
            while ( test_substring_position != string::npos ) {
                ...
                // cout << endl << temp_phrase; // debugging
            ...
            // cout << endl << endl << test_phrase << endl << endl;
            data_of_file.similar_phrase_count++;
                
            similar_phrase_data[0][data_of_file.similar_phrase_count] = to_string(i);
            similar_phrase_data[1][data_of_file.similar_phrase_count] = test_phrase;
    ...
    for (i = 1; i <= data_of_file.similar_phrase_count; i++) {
        cout << endl << "    " << letters.at( (i - 1) % letters.size() ) << ")  ";
        cout << setw(27) << std::left << similar_phrase_data[1][i];
        cout << " " << similar_phrase_data[0][i];

        test_file << endl << "    " << letters.at( (i - 1) % letters.size() ) << ")  ";
        test_file << setw(27) << std::left << similar_phrase_data[1][i];
        test_file << " " << similar_phrase_data[0][i];
    }
}
